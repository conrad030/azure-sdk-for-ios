...
function Get-GithubReferenceCommitDate($commitUrl) {
  try {
    $commitResponse = Invoke-RestMethod $commitUrl
    if (!$commitResponse.committer -or !$commitResponse.committer.date) {
      LogDebug "No date returned from the commit sha. Skipping..."
    }
    return $commitData.committer.date
  }
  catch {
    LogError "Failed to invoke $commitUrl with exception:`n$_"
    exit 1
  }
}
